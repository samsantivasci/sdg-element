  ...
  if (nodeParams['items']) {
    ...
    for (i = 0; i < nodeParams['items'].length; i++) {
      ...
      if (nodeParams['items'][i]['delete'] && nodeParams['items'][i]['delete'] == true) {
        ...
        item = site.addPage(nodeParams['items'][i]['parent'], nodeParams['items'][i]['title'], 'html', nodeParams['items'][i]['slug'], nodeParams['items'][i]['id'], nodeParams['items'][i]['indent'], nodeParams['items'][i]['contents']);
      }
    }
    ...
    if (nodeParams['node']['duplicate']) {
      ...
      if (nodeToDuplicate = site.loadNode(nodeParams['node']['duplicate'])) {
        ...
        if (page = site.loadNode(item.id)) {
          ...
          let bytes = await page.writeLocation(content, site.siteDirectory);
        }
      }
    }
    ...
    // html which becomes the boilerplated content in effect
      ...
      if (page = site.loadNode(item.id)) {
        // write it to the file system
        let bytes = await page.writeLocation(nodeParams['node']['contents'], site.siteDirectory);
      }
    }
