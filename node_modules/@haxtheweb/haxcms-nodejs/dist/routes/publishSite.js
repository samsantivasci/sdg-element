...
async function publishSite(req, res) {
  ...
  if (site) {
    ...
    if (gitSettings) {
      ...
      if (gitSettings.url && gitSettings.url != '' && gitSettings.url != false && gitSettings.url != '/' + site.manifest.metadata.site.name + '.git') {
        domain = gitSettings.url;
        if (site.manifest.metadata.site.domain && site.manifest.metadata.site.domain != '') {
          domain = site.manifest.metadata.site.domain;
        } else {
          // support blowing up github addresses correctly
          parts = explode('/', domain.replace('git@github.com:', '').replace('.git', ''));
          if (parts.length === 2) {
      ...
      // implies the domain is actually on the system locally
        ...
        domain = HAXCMS.basePath + HAXCMS.publishedDirectory + '/' + site.manifest.metadata.site.name + '/';
      }
      ...
      // sanity check
      if (fs.lstatSync(HAXCMS.boilerplatePath + 'cdns/' + cdn + '.html').isFile()) {
        ...
        if (cdn == 'custom') {
          fs.copy(HAXCMS.HAXCMS_ROOT + 'babel/babel-top.js', site.siteDirectory + '/assets/babel-top.js');
          fs.copy(HAXCMS.HAXCMS_ROOT + 'babel/babel-bottom.js', site.siteDirectory + '/assets/babel-bottom.js');
          fs.mirror(HAXCMS.HAXCMS_ROOT + 'build', site.siteDirectory + '/build');
        }
        ...
        let templateVars = {
          ...
          'basePath': '/' + site.manifest.metadata.site.name + '/',
        ...
        if (cdn != 'custom') {
          ...
          templateVars['cdnRegex'] = "(https?:\/\/" + templateVars['cdn'].replace('.', '\.') + `(\/[A-Za-z0-9\-\._~:\/\?#\[\]@!$&'\(\)\*\+,;\=]*)?)`;
        ...
        if (site.manifest.metadata.theme.variables.hexCode) {
          templateVars['hexCode'] = site.manifest.metadata.theme.variables.hexCode;
        }
        let swItems = site.manifest.items;
        // the core files you need in every SW manifest
        coreFiles = ['index.html', 'manifest.json', 'site.json', site.getLogoSize('512', '512'), site.getLogoSize('256', '256'), site.getLogoSize('192', '192'), site.getLogoSize('144', '144'), site.getLogoSize('96', '96'), site.getLogoSize('72', '72'), site.getLogoSize('48', '48'), site.getLogoSize('36', '36'), '404.html'];
        ...
        if (handle) {
          while (false !== (file = readdir(handle))) {
            if (file != "." && file != ".." && file != '.gitkeep' && file != '.DS_Store') {
              ...
              if (fs.lstatSync(site.siteDirectory + '/files/' + file).isFile()) {
        ...
        for (var key in swItems) {
          ...
          if (item.location === '' || item.location === templateVars['basePath']) {
            filesize = filesize(site.siteDirectory + '/index.html');
          } else if (fs.lstatSync(site.siteDirectory + '/' + item.location).isFile()) {
            filesize = filesize(site.siteDirectory + '/' + item.location);
          } else {
      ...
      fs.mirror(site.siteDirectory, HAXCMS.configDirectory + '/../_published/' + site.manifest.metadata.site.name);
      // remove the .git version control from this, it's not needed
      fs.rmdir(HAXCMS.configDirectory + '/../_published/' + site.manifest.metadata.site.name + '/.git');
      // rewrite the base path to ensure it is accurate based on a local build publish vs web
      let index = await fs.readFileSync(HAXCMS.configDirectory + '/../_published/' + site.manifest.metadata.site.name + '/index.html', {
        encoding: 'utf8',
        flag: 'r'
      });
      // replace if it was publishing with the name in it
      index = index.replace('<base href="/' + site.manifest.metadata.site.name + '/"', '<base href="' + HAXCMS.basePath + '_published/' + site.manifest.metadata.site.name + '/"');
      // replace if it has a vanity domain
      index = index.replace('<base href="/"', '<base href="' + HAXCMS.basePath + '_published/' + site.manifest.metadata.site.name + '/"');
      // rewrite the file
      fs.writeFileSync(HAXCMS.configDirectory + '/../_published/' + site.manifest.metadata.site.name + '/index.html', index);
      // tag, attempt to push, and set things up for next time
      repo.add_tag('version-' + site.manifest.metadata.site.static.lastPublished);
      repo.push('origin', 'version-' + site.manifest.metadata.site.static.lastPublished, "--force");
      if (gitSettings.staticBranch != 'master') {
      ...
      if (!fs.lstatSync(site.siteDirectory + '/node_modules').isSymbolicLink()) {
        fs.symlink('../../node_modules', site.siteDirectory + '/node_modules');
      }
      if (fs.lstatSync(site.siteDirectory + '/assets/babel-top.js').isSymbolicLink()) {
        fs.unlink(site.siteDirectory + '/assets/babel-top.js');
      }
      ...
      fs.symlink('../../../babel/babel-top.js', site.siteDirectory + '/assets/babel-top.js');
      fs.symlink('../../../babel/babel-bottom.js', site.siteDirectory + '/assets/babel-bottom.js');
